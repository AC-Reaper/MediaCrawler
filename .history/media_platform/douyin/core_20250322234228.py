...
class DouYinCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            elif config.CRAWLER_TYPE == "user_search":  # 新增的用户搜索类型
        # 搜索用户并获取用户信息
    ...
    async def search_users(self):
        """搜索用户并保存用户数据"""
        utils.logger.info("[DouYinCrawler.search_users] 开始搜索抖音用户")
        for keyword in config.KEYWORDS.split(","):
            source_keyword_var.set(keyword)
            utils.logger.info(f"[DouYinCrawler.search_users] 当前搜索关键词: {keyword}")
            
            page = 0
            max_users_count = config.CRAWLER_MAX_NOTES_COUNT  # 复用视频数量限制
            users_list = []
            
            while page * 12 < max_users_count:
                try:
                    utils.logger.info(f"[DouYinCrawler.search_users] 搜索抖音用户: {keyword}, 页码: {page}")
                    users_res = await self.dy_client.search_users_by_keyword(
                        keyword=keyword,
                        offset=page * 12,
                        count=12
                    )
                    
                    if "user_list" not in users_res:
                        utils.logger.error(f"[DouYinCrawler.search_users] 搜索抖音用户: {keyword} 失败，账号可能被风控")
                        break
                        
                    user_list = users_res.get("user_list", [])
                    if not user_list:
                        utils.logger.info(f"[DouYinCrawler.search_users] 没有更多用户数据")
                        break
                    ...
                    for user_item in user_list:
                        ...
                        # 保存用户数据
                        await douyin_store.save_creator(user_info.get("uid", ""), {"user": user_info})
                        users_list.append(sec_user_id)
                        
                    page += 1
                    await asyncio.sleep(random.uniform(1, 3))  # 添加随机延迟防止被封
                    
                except Exception as e:
                    utils.logger.error(f"[DouYinCrawler.search_users] 搜索出错: {e}")
                    break
                    
            # 如果配置了爬取用户视频，可以继续获取这些用户的视频
            if config.ENABLE_GET_USER_VIDEOS and users_list:
                await self.get_users_videos(users_list)