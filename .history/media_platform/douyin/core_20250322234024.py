...
class DouYinCrawler(AbstractCrawler):
    ...
    搜索抖音用户
    """
    utils.logger.info(f"[DouYinCrawler.search_users] 开始搜索抖音用户: {keyword}")
    try:
        # 调用用户搜索API
        user_results = await self.dy_client.search_users_by_keyword(
            keyword=keyword,
            offset=0,
            count=12
        )
        ...
        # 处理返回的用户数据
        if "user_list" in user_results:
            users = user_results.get("user_list", [])
            for user_item in users:
                # 提取用户信息
                user_info = user_item.get("user_info", {})
                # 保存用户数据
                await douyin_store.save_creator(
                    user_id=user_info.get("uid", ""),
                    creator={"user": user_info}
                )
            utils.logger.info(f"[DouYinCrawler.search_users] 搜索到 {len(users)} 个用户")
        else:
            utils.logger.error(f"[DouYinCrawler.search_users] 搜索失败，返回数据格式不正确")
    except Exception as e:
        utils.logger.error(f"[DouYinCrawler.search_users] 搜索异常: {e}")