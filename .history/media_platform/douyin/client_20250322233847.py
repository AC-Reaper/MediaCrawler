...
class DOUYINClient(AbstractApiClient):
    ...
    query_params = {
        ...
        'aid': '6383',
        'channel': 'channel_pc_web',
        'search_channel': 'aweme_user_web',  # 用户搜索通道
        'keyword': keyword,  # 关键词会在请求时自动进行URL编码
    ...
    # 处理a_bogus参数
    # 此参数需要通过JavaScript计算，类似于self.__process_req_params中的处理
    headers = copy.copy(self.headers)
    referer_url = f"https://www.douyin.com/search/{urllib.parse.quote(keyword)}?type=user"
    headers["Referer"] = referer_url
    
    # 调用请求处理方法获取a_bogus
    await self.__process_req_params(uri, query_params, headers)
